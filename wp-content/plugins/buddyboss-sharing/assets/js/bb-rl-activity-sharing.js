...
(function ($) {
  ...
  var BuddyBossActivitySharing = {
    ...
    bindEvents: function bindEvents() {
      ...
      $(document).on('reset', '#bb-rl-whats-new-form', this.handleFormReset.bind(this));
      ...
      $(document).on('click', '.bb-rl-activity-update-form.modal-popup.bb-rl-activity-edit #bb-rl-aw-whats-new-reset', this.cleanupEditActivityPreview.bind(this));
      ...
      $(document).on('click', '.bb-rl-activity-content .shared-activity-preview, .shared-activity-message .shared-activity-preview', this.handleSharedActivityClick.bind(this));
      ...
      var mediaClickHandler = function mediaClickHandler(e) {
        ...
        // Check if the click is inside a shared activity preview
        ...
        // Special handling for .bb-rl-open-video-theatre and .bb-open-video-theatre
        ...
        if ($target.is('a.bb-rl-open-video-theatre, a.bb-open-video-theatre') || $target.closest('a.bb-rl-open-video-theatre, a.bb-open-video-theatre').length > 0) {
          if (isInMessages) {
        ...
        if ($target.is('.vjs-icon-square, .enlarge_button') || $target.closest('.vjs-icon-square, .enlarge_button').length > 0) {
          // The BuddyBoss handler will trigger a click on .bb-rl-open-video-theatre (ReadyLaunch) or .bb-open-video-theatre
          ...
          // Check for ReadyLaunch class first, then fallback to regular
          var $videoTheatreLink = $videoContainer.find('.bb-rl-open-video-theatre, .bb-open-video-theatre');
        ...
        if (isInMessages) {
          ...
          if ($videoWrap.length > 0 && $videoWrap.hasClass('bb-video-length-1')) {
            ...
            var isVideoElement = $target.is('video, .video-js, .single-activity-video, .bb-rl-single-activity-video') || $target.closest('video, .video-js, .single-activity-video, .bb-rl-single-activity-video, .vjs-control-bar, .vjs-big-play-button').length > 0;
        ...
        var isMediaElement = $target.is('a.bb-rl-open-media-theatre, a.bb-open-document-theatre, a.bb-rl-open-video-theatre, a.bb-open-video-theatre, a.bb-rl-entry-img, img, video, audio') || $target.closest('a.bb-rl-open-media-theatre, a.bb-open-document-theatre, a.bb-rl-open-video-theatre, a.bb-open-video-theatre, a.bb-rl-entry-img, .bb-rl-activity-media-elem, .document-activity, .media-activity, .bb-activity-video-wrap, .bb-activity-video-elem, .bb-rl-activity-video-elem').length > 0;
    ...
    closeModal: function closeModal(e) {
      ...
      if (this.currentActivityId && (this.currentShareType === 'feed' || this.currentShareType === 'group' || this.currentShareType === 'profile')) {
        ...
        if ($('#bb-rl-whats-new-form').find('.buddyboss-shared-activity-preview').length > 0) {
    ...
    openActivityModal: function openActivityModal() {
      ...
      var $formContainer = $('#bb-rl-activity-form');
    ...
    loadActivityPostForm: function loadActivityPostForm() {
      return new Promise(function (resolve, reject) {
        ...
        var $existingForm = $('#bb-rl-whats-new-form');
        ...
        var $formContainer = $('#bb-rl-activity-form');
        var $editFormWrap = $('#bb-rl-single-activity-edit-form-wrap');
        if (!$formContainer.length) {
          // Create form container in the single activity edit form wrap or body
          if ($editFormWrap.length) {
            $formContainer = $('<div id="bb-rl-activity-form" class="bb-rl-activity-update-form"></div>');
        ...
        $.ajax({
          ...
          success: function success(response) {
            if (response.success && response.data.form_html) {
              ...
              // The platform's JS checks for .bb-rl-activity-update-form on page load
                ...
                if ($form.length) {
                  ...
                  if (typeof bp !== 'undefined' && bp.Nouveau && bp.Nouveau.Activity && bp.Nouveau.Activity.postForm && typeof bp.Nouveau.Activity.postForm.postFormView === 'function') {
                    ...
                    try {
                      ...
                      setTimeout(function () {
                        if ($('#bb-rl-whats-new-form').length) {
                          resolve();
                        } else {
    ...
    openBuddyBossPostForm: function openBuddyBossPostForm() {
      var $textarea = $('#bb-rl-whats-new');
      var $form = $('#bb-rl-whats-new-form');
      var $formWrapper = $form.closest('.bb-rl-activity-update-form');
      var $editFormWrap = $('#bb-rl-single-activity-edit-form-wrap');
      ...
      if ($textarea.length) {
        ...
        setTimeout(function () {
          ...
          if ($formWrapper.length && !$formWrapper.hasClass('modal-popup')) {
            ...
            $('body').addClass('bb-rl-activity-modal-open');
          }
          ...
          // Focus the textarea to trigger ReadyLaunch's displayFull
          $textarea.focus();
        }, 100);
      } else {
    ...
    disableUploadButtons: function disableUploadButtons() {
      ...
      setTimeout(function () {
        var $toolbar = $('#bb-rl-whats-new-toolbar');
        if ($toolbar.length) {
          $toolbar.attr('data-share-disabled', 'true');
        } else {
          // Try again if not found
          setTimeout(function () {
            var $toolbar2 = $('#bb-rl-whats-new-toolbar');
            if ($toolbar2.length) {
    ...
    hideEditFormWrapOnSingleActivityPage: function hideEditFormWrapOnSingleActivityPage() {
      ...
      if ($editFormWrap.length && $('body').hasClass('activity-singular')) {
        ...
        var $formWrapper = $editFormWrap.find('.bb-rl-activity-update-form');
        if ($formWrapper.length) {
          $formWrapper.removeClass('modal-popup');
          $('body').removeClass('bb-rl-activity-modal-open');
        }
      }
    },
    /**
    ...
    hookFormSubmission: function hookFormSubmission() {
      ...
      $('#bb-rl-whats-new-form').off('submit.buddyboss-share');

      // Add new handler with native addEventListener to bind at capture phase
      var form = document.getElementById('bb-rl-whats-new-form');
      if (form) {
        ...
        form._buddybossShareHandler = function (e) {
          ...
          if (sharedActivityId && sharedActivityId.value) {
            ...
            var privacy = $('#bb-rl-whats-new-form').find('.bp-activity-privacy__input:checked').val() || 'public';
        ...
        console.error('Form #bb-rl-whats-new-form not found!');
      }
    },
    /**
    ...
    submitSharedActivity: function submitSharedActivity(activityId, customMessage, privacy) {
      ...
      // Extract topic_id and has_topic_selector if topic selector exists (ReadyLaunch uses .bb-rl-topic-selector-list)
      var topicId = 0;
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success) {
            ...
            // Inject activity into stream (ReadyLaunch version)
            if (response.data.activity && response.data.id) {
              var _activityId = response.data.id;
              var activityElemSel = $('#bb-rl-activity-' + _activityId);
              ...
              if (!activityElemSel.length) {
                ...
                var pinnedActivity = $('#bb-rl-activity-stream ul.bb-rl-activity-list li:first.bb-pinned');
                if (pinnedActivity.length > 0) {
                  ...
                  if (typeof bp !== 'undefined' && bp.Nouveau && bp.Nouveau.inject) {
                    bp.Nouveau.inject('#bb-rl-activity-stream ul.bb-rl-activity-list li:first.bb-pinned', response.data.activity, 'after');
                  } else {
                  ...
                  if (typeof bp !== 'undefined' && bp.Nouveau && bp.Nouveau.inject) {
                    bp.Nouveau.inject('#bb-rl-activity-stream ul.bb-rl-activity-list', response.data.activity, 'prepend');
                  } else {
                    $('#bb-rl-activity-stream ul.bb-rl-activity-list').prepend(response.data.activity);
                  }
                }
            ...
            $('#bb-rl-whats-new-form').trigger('reset');
          } else {
    ...
    cleanupSharedActivityForm: function cleanupSharedActivityForm() {
      ...
      $('#bb-rl-whats-new-form').off('submit.buddyboss-share');
      $('#bb-rl-whats-new-form').removeClass('activity-share-form');
      ...
      // Show toolbar again
      var $toolbar = $('#bb-rl-whats-new-toolbar[data-share-disabled="true"]');
      if ($toolbar.length) {
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success) {
            ...
            // Prepend activity to feed if activity HTML is provided (same as BuddyBoss Platform)
            if (response.data.activity && response.data.id) {
              var activityId = response.data.id;
              var activityElemSel = $('#bb-rl-activity-' + activityId);
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success) {
            ...
            if (response.data.activity && response.data.id && !isSingleActivityPage) {
              var activityId = response.data.id;
              var activityElemSel = $('#bb-rl-activity-' + activityId);
            ...
            if (isSingleActivityPage && response.data.id) {
              // Remove the activity if it was added (ReadyLaunch selectors)
              $('#bb-rl-activity-' + response.data.id).remove();
              // Also check for any activities with the share action type
              $('#bb-rl-activity-stream li.activity-item[data-bp-activity-id="' + response.data.id + '"]').remove();
            }
          } else {
    ...
    handleFormReset: function handleFormReset(e) {
      ...
      var $form = $('#bb-rl-whats-new-form');
      if ($form.hasClass('bb-rl-activity-edit') || $form.closest('.bb-rl-activity-update-form, .activity-update-form').hasClass('bb-rl-activity-edit')) {
    ...
    handleGroupSelection: function handleGroupSelection(groupId, groupName, groupAvatar) {
      ...
      if (buddybossSharingFrontend.enableCustomMsg) {
        ...
        // Load and inject shared activity content
        this.loadAndInjectSharedActivityWithGroup();
      } else {
        // Share directly without modal
            ...
            if (page < totalPages) {
              ...
              if (allTopics.length >= 0) {
                ...
                if (model) {
                  ...
                  // ReadyLaunch uses different selector
    ...
    hookFormSubmissionForGroup: function hookFormSubmissionForGroup() {
      var self = this;
      // Remove previous handler if exists
      $('#bb-rl-whats-new-form').off('submit.buddyboss-share-group');
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success) {
            ...
            if (response.data.activity && response.data.id) {
              var _activityId2 = response.data.id;
              var activityElemSel = $('#bb-rl-activity-' + _activityId2);
    ...
    handleFriendSelection: function handleFriendSelection(userId, userName, userAvatar) {
      ...
      if (buddybossSharingFrontend.enableCustomMsg) {
        ...
        // Load and inject shared activity content
        this.loadAndInjectSharedActivityWithFriend();
      } else {
        // Share directly without modal
    ...
    hookFormSubmissionForFriend: function hookFormSubmissionForFriend() {
      ...
      $('#bb-rl-whats-new-form').off('submit.buddyboss-share-friend');
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success && response.data) {
            ...
            // Ensure the activity item has the correct ID (ReadyLaunch uses bb-rl-activity- prefix)
            tempActivity.attr('id', 'bb-rl-activity-' + activityId);
    ...
    hookEditFormSubmission: function hookEditFormSubmission() {
      ...
      if (form) {
        ...
        form._buddybossShareEditHandler = function (e) {
          var sharedActivityId = document.getElementById('buddyboss-shared-activity-id');
          var activityId = document.getElementById('bb-rl-activity-id');
    ...
    cleanupEditActivityPreview: function cleanupEditActivityPreview() {
      var $form = $('#bb-rl-whats-new-form');
      var $formWrapper = $form.closest('.bb-rl-activity-update-form, .activity-update-form');
      ...
      // Or if the form doesn't have bb-rl-activity-edit class (edit mode ended)
      if ($formWrapper.length && (!$formWrapper.hasClass('modal-popup') || !$form.hasClass('bb-rl-activity-edit'))) {
    ...
    setupEditModalCleanupObserver: function setupEditModalCleanupObserver() {
      ...
      this.editModalObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            ...
            if (($target.hasClass('bb-rl-activity-update-form') || $target.hasClass('activity-update-form')) && !$target.hasClass('modal-popup')) {
              // Check if there's a preview in the form
              var $form = $target.find('#bb-rl-whats-new-form');
      ...
      $(document).ready(function () {
        ...
        $('.bb-rl-activity-update-form, .activity-update-form').each(function () {
          observeForm(this);
        });
        ...
        var bodyObserver = new MutationObserver(function (mutations) {
          mutations.forEach(function (mutation) {
            $(mutation.addedNodes).find('.bb-rl-activity-update-form, .activity-update-form').addBack('.bb-rl-activity-update-form, .activity-update-form').each(function () {
              observeForm(this);
            });
          });
        });
...
/******/ })()
;
//# sourceMappingURL=bb-rl-activity-sharing.js.map