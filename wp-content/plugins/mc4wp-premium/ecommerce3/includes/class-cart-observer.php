...
class MC4WP_Ecommerce_Cart_Observer
{
    ...
    {
        add_action('parse_request', [ $this, 'repopulate_cart_from_mailchimp' ]);
        add_action('wp_enqueue_scripts', [ $this, 'enqueue_assets' ]);
        add_action('wp_ajax_mc4wp_ecommerce_schedule_cart', [ $this, 'on_checkout_form_change' ]);
        add_action('wp_ajax_nopriv_mc4wp_ecommerce_schedule_cart', [ $this, 'on_checkout_form_change' ]);
        add_action('woocommerce_order_status_changed', [$this, 'on_order_status_change'], 10, 4);
        add_action('woocommerce_after_cart_item_restored', [ $this, 'on_cart_updated' ]);
        add_action('woocommerce_after_cart_item_quantity_update', [ $this, 'on_cart_updated' ]);
        add_action('woocommerce_add_to_cart', [ $this, 'on_cart_updated' ], 9);
        add_action('woocommerce_cart_item_removed', [ $this, 'on_cart_updated' ]);
        add_action('woocommerce_cart_emptied', [ $this, 'on_cart_updated' ]);
        add_action('wp_login', [ $this, 'on_cart_updated' ], 10, 2);
    }
    ...
    public function enqueue_assets()
    {
        ...
        wp_enqueue_script('mc4wp-ecommerce-cart', plugins_url("/assets/js/cart.js", $this->plugin_file), [], MC4WP_PREMIUM_VERSION, true);
    ...
    {
        if ($handle !== 'mc4wp-ecommerce-cart' || strpos($tag, 'defer') !== false) {
            return $tag;
        }
    ...
    public function on_checkout_form_change()
    {
        ...
        if (! $customer_data || empty($customer_data->billing_email)) {
            wp_send_json_error();
            exit;
        }
        ...
        if (is_email($customer_data->billing_email)) {
            ...
            if (! $this->is_cart_empty($wc_cart)) {
                ...
                $this->add_pending_job('update_cart', [$cart_id, $customer_data, $cart_contents]);
            } else {
                $this->add_pending_job('delete_cart', [$cart_id]);
            }
        }
        ...
        if (! empty($customer_data->previous_billing_email) && $customer_data->previous_billing_email !== $customer_data->billing_email) {
            ...
            $this->add_pending_job('delete_cart', [ $previous_cart_id ]);
        }
    ...
    public function on_order_status_change($order_id, $from, $to, $order)
    {
        ...
        $order_statuses_to_keep = ['', 'pending', 'failed'];
        if (in_array($to, $order_statuses_to_keep, true)) {
            return;
        }

        // delete cart in Mailchimp if coming from pending or failed status
        if (! in_array($from, $order_statuses_to_keep, true)) {
            return;
        }
    ...
    public function on_cart_updated($a = null, $b = null)
    {
        ...
        $customer = $user_id > 0 ? $user_id : (object) [
        ...
        // add data to queue
        $this->add_pending_job('update_cart', [$cart_id, $customer, $cart_contents]);
        $this->queue->save();
    }
