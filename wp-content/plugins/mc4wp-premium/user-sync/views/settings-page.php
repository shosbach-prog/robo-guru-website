    ...
    <div class="mc4wp-row">
        ...
        <div class="main-content mc4wp-col">
            ...
            <form method="post" id="settings-form">
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <?php if (empty($lists)) {
                                printf(wp_kses(__('No lists found, <a href="%s">are you connected to MailChimp</a>?', 'mailchimp-for-wp'), [ 'a' => [ 'href' => '' ] ]), admin_url('admin.php?page=mailchimp-for-wp')); ?>
                            <?php } else { ?>
                            <select name="<?php echo $this->name_attr('list'); ?>" class="widefat">
                                <option disabled <?php selected($this->options['list'], ''); ?>><?php echo esc_html__('Select an audience..', 'mailchimp-sync'); ?></option>
                    ...
                    <tr valign="top">
                        ...
                        <td class="mc4wp-sync-field-map">
                            ...
                            if (! isset($selected_list)) {
                                echo '<p class="description">' , esc_html__('Please select a MailChimp audience first (and then save your settings).', 'mailchimp-sync') , '</p>';
                            } else {
                                echo '<div>';
                                ...
                                <div class="field-map-row" style="margin-bottom: 4px;">
                                    ...
                                    <input type="button" value="&times;" class="button remove-row" title="<?php esc_attr_e('Delete row', 'mailchimp-sync'); ?>" aria-label="<?php esc_attr_e('Delete row', 'mailchimp-sync'); ?>" />
                                </div>
                                    <?php
                                } // end foreach
                                ?>
                                </div>
                                <p><input type="button" class="button add-row" value="&plus; <?php echo esc_attr__('Add row', 'mailchimp-sync'); ?>" style="margin-left:0; "/></p>

                                <p class="description" style="margin-top: 12px;">
                                    <?php printf(wp_kses(__('Specify <a href="%s">what Mailchimp field each user fields maps to</a>. Email address is always synchronized.', 'mailchimp-sync'), [ 'strong' => [], 'a' => [ 'href' => [] ] ]), 'https://www.mc4wp.com/kb/syncing-custom-user-fields-mailchimp/#utm_source=wp-plugin&utm_medium=mailchimp-sync&utm_campaign=settings-page', '"user meta"'); ?>
                                </p>
                                <p class="description">
            ...
            <?php if ($this->options['enabled'] && '' !== $this->options['list']) { ?>
                ...
                echo '<p>';
                echo sprintf(wp_kses(__('There are <strong>%d</strong> background jobs waiting to be processed.', 'mailchimp-sync'), [ 'strong' => [] ]), $number_of_pending_jobs);
                ...
                if ($number_of_pending_jobs > 0) {
                    echo '<p><a class="button" href="', esc_attr(add_query_arg([ '_mc4wp_action' => 'process_user_sync_queue', '_wpnonce' => wp_create_nonce('_mc4wp_action') ])), '">', esc_html__('Process', 'mailchimp-sync'), '</a></p>';
                }

                echo '<p class="description">', sprintf(wp_kses(__('Keep an eye on the <a href="%s">debug log</a> for any errors in the background sync process.', 'mailchimp-sync'), [ 'a' => [ 'href' => [] ] ]), admin_url('admin.php?page=mailchimp-for-wp-other')), '</p>';
...
<template id="field-map-row-template">
<div class="field-map-row" style="margin-bottom: 4px;">
    <select class="user-field" name="<?php echo $this->name_attr('[field_map][0][user_field]'); ?>">
        <option value="" disabled><?php echo esc_html__('User field', 'mailchimp-sync'); ?></option>
    ...
    <select name="<?php echo $this->name_attr('[field_map][0][mailchimp_field]'); ?>" class="mailchimp-field">
        <option value="" disabled><?php echo esc_html__('MailChimp field', 'mailchimp-sync'); ?></option>
        <?php foreach ($available_mailchimp_fields as $field) { ?>
            <?php if ($field->tag === 'EMAIL') {
                continue;
            } ?>
            <option value="<?php echo esc_attr($field->tag); ?>">
