		...
		foreach ($this->parameters as $k => $v) {
		  ...
		  if (is_array($v)) {
			throw new Plus_OAuthException(
				__( 'Arrays not supported in headers', 'ultimate-social-media-plus' )
			);
		  }
		  $out .= ($first) ? ' ' : ',';
	...
	class Plus_OAuthServer {
	  ...
	  private function get_version(&$request) {
		...
		if ($version !== $this->version) {
		  throw new Plus_OAuthException(
		  	sprintf(
				__( 'OAuth version "%1$s" not supported', 'ultimate-social-media-plus' ),
				$version
			)
	  ...
	  private function get_signature_method(&$request) {
		...
		if (!$signature_method) {
		  ...
		  throw new Plus_OAuthException(
		  	__( 'No signature method parameter. This parameter is required', 'ultimate-social-media-plus' )
		  );
		}
	  ...
	  private function get_consumer(&$request) {
		...
		if (!$consumer) {
		  throw new Plus_OAuthException(
		  	__( 'Invalid consumer', 'ultimate-social-media-plus' )
		  );
		}
	  ...
	  private function get_token(&$request, $consumer, $token_type="access") {
		...
		if (!$token) {
		  throw new Plus_OAuthException(
		  	sprintf(
				__( 'Invalid %1$s token: %2$s', 'ultimate-social-media-plus' ),
				$token_type,
				$token_field
			)
		  );
	  ...
	  private function check_signature(&$request, $consumer, $token) {
		...
		if (!$valid_sig) {
		  	throw new Plus_OAuthException(
				__( 'Invalid signature', 'ultimate-social-media-plus' )
			);
		}
	  }
	  ...
	  private function check_timestamp($timestamp) {
		if( ! $timestamp )
		  throw new Plus_OAuthException(
			__( 'Missing timestamp parameter. The parameter is required', 'ultimate-social-media-plus' ),
		  );
		...
		if (abs($now - $timestamp) > $this->timestamp_threshold) {
		  throw new Plus_OAuthException(
		  	sprintf(
				__( 'Expired timestamp, yours %1$s, ours %2$s', 'ultimate-social-media-plus' ),
				$timestamp,
				$now
			)
		  );
	  ...
	  private function check_nonce($consumer, $token, $nonce, $timestamp) {
		if( ! $nonce )
		  throw new Plus_OAuthException(
			__( 'Missing nonce parameter. The parameter is required', 'ultimate-social-media-plus' )
		  );
		...
		if ($found) {
		  throw new Plus_OAuthException(
		  	sprintf(
				__( 'Nonce already used: %s', 'ultimate-social-media-plus' ),
				$nonce
			)
		  );
		}
