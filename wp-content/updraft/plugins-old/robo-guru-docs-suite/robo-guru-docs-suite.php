...
function robot_frontend_assets(){
    ...
    $css = '
    .rg-docs-wrap{margin-top:22px;padding:16px;border:1px solid rgba(0,0,0,.10);border-radius:16px;background:#fff;box-shadow:0 10px 24px rgba(0,0,0,.05)}
    ...
    .rg-docs-item__actions{display:flex;gap:8px;flex:0 0 auto}
    ...
    .rg-docs-modal__box{width:min(1100px, 96vw);height:min(760px, 88vh);background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,.25);display:flex;flex-direction:column}
    .rg-docs-modal__top{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border-bottom:1px solid rgba(0,0,0,.08)}
    .rg-docs-modal__title{font-weight:650}
    .rg-docs-modal__close{cursor:pointer;border:1px solid rgba(0,0,0,.10);border-radius:10px;background:#fff;padding:6px 10px}
    .rg-docs-modal__frame{flex:1 1 auto}
    .rg-docs-modal__frame iframe{width:100%;height:100%;border:0}
    ';
    ...
    (function(){
      ...
      modal.innerHTML = '<div class=\"rg-docs-modal__box\">'
        ...
        + '</div>'
        + '<div class=\"rg-docs-modal__frame\"><iframe loading=\"lazy\"></iframe></div>'
        + '</div>';
      ...
      qs('.rg-docs-modal__close', modal).addEventListener('click', close);
      ...
      document.addEventListener('click', function(e){
        ...
        qs('.rg-docs-modal__title', modal).textContent = titleEl ? titleEl.textContent.trim() : 'Vorschau';
        qs('iframe', modal).src = a.href;
          ...
          <details class="rg-docs-folder" open>
            ...
            <div class="rg-docs-list">
              <?php foreach ($items as $it): ?>
                ...
                ?>
                <div class="rg-docs-item" data-ext="<?php echo esc_attr($ext); ?>">
  ...
  private function collect_robot_documents($post_id){
    ...
    if (($mode === 'bb' || $mode === 'all') && function_exists('buddypress') && class_exists('BP_Document')) {
      ...
      if (!empty($bb_folder_arr)) {
        ...
        foreach ($rows as $row) {
          if (!$this->bb_is_doc_visible($row)) continue;
    ...
    $out = array('groups'=>$groups, 'flat'=>$flat, 'mode'=>$mode, 'cached_at'=>time());
  ...
  private function bb_get_doc_row_cached($doc_id){
    ...
    if ($ttl > 0) {
      $cached = get_transient($key);
      if (is_array($cached) && isset($cached['id'])) return $cached;
    }
  ...
  public function docs_frontend_assets() {
    ...
    $css = '
      .rg-docs-wrap{display:flex;flex-wrap:wrap;gap:10px;margin:12px 0}
