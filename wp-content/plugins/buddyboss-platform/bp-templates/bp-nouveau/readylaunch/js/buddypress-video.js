...
( function ( exports, $ ) {
	...
	bp.Nouveau.Video = {
		...
		setupGlobals: function () {
			...
			var uploaderVideoTemplate = document.getElementsByClassName( 'uploader-post-video-template' ).length ? document.getElementsByClassName( 'uploader-post-video-template' )[0].innerHTML.trim() : ''; // Check to avoid error if Node is missing.
			this.videoOptions         = {
				url: bbRlAjaxUrl,
		...
		openEditThumbnailUploader : function ( event ) {
			...
			if ( typeof window.Dropzone !== 'undefined' && $( 'div.bb-rl-video-thumbnail-uploader.opened-edit-thumbnail div.bb-rl-video-thumbnail-uploader-wrapper .bb-rl-video-thumbnail-uploader-dropzone-select' ).length ) {
				...
				self.videoThumbnailOptions.previewTemplate = document.getElementsByClassName( 'bb-rl-uploader-post-video-thumbnail-template' ).length ? document.getElementsByClassName( 'bb-rl-uploader-post-video-thumbnail-template' )[ 0 ].innerHTML.trim() : '';
