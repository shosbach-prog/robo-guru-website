...
- User Sync: fix function call for setting up cron schedule when saving settings.


#### 4.10.22 - Feb 20, 2026

- Fix "Cannot modify header information" when accessing User Sync settings page without having activated your MC4WP Premium license.
- E-commerce: Automatically translate WPML product URL's.


#### 4.10.21 - Feb 17, 2026

- E-commerce: fix error upon plugin activation when activating plugin without WooCommerce active.


#### 4.10.20 - Feb 11, 2026

- E-commerce: Properly remove scheduled hooks upon plugin deactivation
- E-commerce: Properly re-instantiate scheduled hooks upon plugin activation (if needed).
- E-commerce: Disconnect store from Mailchimp when running setup wizard whilst choosing a new Mailchimp audience.
- E-commerce: Fix white screen when encountering API errors whilst setting up store connection.
- User-sync: Properly re-instantiate scheduled hooks upon plugin activation (if needed).
- User-sync: Fix PHP warning about undefined variable when accessing settings page on a fresh installation.


#### 4.10.19 - Dec 17, 2025

- Minor performance improvements to plugin bootstrapping code.
- Replace chart library from dashboard widget showing Audience size/activity with a locally hosted version with a much smaller filesize.


#### 4.10.17 - Sep 23, 2025

- [Logging] Fix total item count not updating when searching log items.
- Minor performance gain by explicitly passing 0 arguments to after_plugins_row callback function.


#### 4.10.16 - Aug, 15, 2025

- [User Sync] Ensure custom cron schedule is registered before setting up scheduled hook.
- Require active plugin license to access certain settings pages.


#### 4.10.15 - Jul 24, 2025

- [E-commerce] Fetch untracked order ID's asynchronically to prevent timing out the entire settings page.
- [E-commerce] Check if cookie name is set before temporarily unsetting admin cookies.
...
- Add filter hook for showing the "create campaign" button on post types other than posts. [Example code snippet](https://github.com/ibericode/mailchimp-for-wordpress/blob/main/sample-code-snippets/premium/post-campaign/add-post-types.php).


#### 4.10.12 - May 12, 2025

- User Sync: don't try to sync user with ID 0. This fixes an error occurring because of Rank Math saving user meta for an unexisting user.


#### 4.10.11 - Apr 14, 2025

- Updated third-party JS dependencies to their latest versions.


#### 4.10.10 - Feb 19, 2025

- [User Sync] Add class autoloader.
- [Autocomplete] Defer script.
- [Styles Builder] Only enqueue wp-color-picker on the specific pages we need it on.
