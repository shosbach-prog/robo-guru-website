...
class Ajax_Listener
{
    ...
    {
        add_action('wp_ajax_mc4wp_user_sync_get_user_count', [ $this, 'get_user_count' ]);
        add_action('wp_ajax_mc4wp_user_sync_get_users', [ $this, 'get_users' ]);
        add_action('wp_ajax_mc4wp_user_sync_handle_user', [ $this, 'handle_user' ]);
    }
    ...
    public function get_users()
    {
        ...
        // get users
        $users = $this->users->get([ 'fields' => [ 'ID', 'user_login', 'user_email' ], 'offset' => $offset, 'number' => $limit ]);
    ...
    public function handle_user()
    {
        ...
        switch ($status) {
            ...
            case User_Handler::NOT_ON_LIST:
                $message = sprintf(__('User %d was not found in Mailchimp audience', 'mailchimp-sync'), $user_id);
                break;
