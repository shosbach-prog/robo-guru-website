...
class MC4WP_Form_Notifications_Admin
{
    ...
    {
        add_action('admin_init', [ $this, 'maybe_run_upgrade_routines' ]);
        add_action('mc4wp_admin_enqueue_assets', [ $this, 'enqueue_assets' ]);
        add_filter('mc4wp_admin_edit_form_tabs', [ $this, 'add_tab' ]);
        add_action('mc4wp_admin_edit_form_output_emails_tab', [ $this, 'output_settings' ], 10, 2);
        add_filter('mc4wp_form_sanitized_data', [ $this, 'sanitize_data' ], 10, 2);
    }
    ...
    public function enqueue_assets($suffix = '', $page = '')
    {
        ...
        wp_enqueue_script('mc4wp-email-notifications', plugins_url("/assets/js/admin.js", $this->plugin_file), [ 'mc4wp-forms-admin' ], MC4WP_PREMIUM_VERSION, true);
    }
