...
class RG_FFmpeg_Video_Poster_Generator {
  ...
  public static function init(): void {
    ...
    if (is_admin()) {
      ...
      // Batch regeneration for existing videos
      add_action('admin_post_rg_ffmpeg_regen_all', [__CLASS__, 'admin_handle_regen_all']);
    }

    // WP-CLI command: wp rg-video-poster regen --force --batch=50
    if (defined('WP_CLI') && WP_CLI) {
      \WP_CLI::add_command('rg-video-poster regen', [__CLASS__, 'cli_regen']);
    }
  }
  ...
  public static function maybe_generate_poster_on_upload(int $attachment_id): void {
    ...
    $force = !empty($settings['force_regen']);
    self::generate_poster($attachment_id, $force);
  }

  /**
   * Generates (or regenerates) a poster for a given video attachment.
   * Returns true on success.
   */
  public static function generate_poster(int $attachment_id, bool $force = false): bool {
    $post = get_post($attachment_id);
    if (!$post || $post->post_type !== 'attachment') return false;

    $mime = get_post_mime_type($attachment_id);
    if (!$mime || strpos($mime, 'video/') !== 0) return false;
    ...
    // If we force-regenerate and an older poster exists, delete it to avoid media clutter
    if ($force) {
      $existing = (int)get_post_meta($attachment_id, self::META_POSTER_ID, true);
      if ($existing > 0) {
        wp_delete_attachment($existing, true);
        delete_post_meta($attachment_id, self::META_POSTER_ID);
        delete_post_meta($attachment_id, self::META_POSTER_URL);
      }
    }
  ...
  /**
   * Admin handler: regenerate posters for existing video attachments in batches.
   * This avoids timeouts by processing e.g. 20 items per request and redirecting.
   */
  public static function admin_handle_regen_all(): void {
    if (!current_user_can('manage_options')) {
      wp_die('No permission');
    }
    check_admin_referer('rg_ffmpeg_regen_all');

    $force = !empty($_POST['force'] ?? '') || !empty($_GET['force'] ?? '');
    $batch = (int)($_POST['batch'] ?? ($_GET['batch'] ?? 20));
    if ($batch < 5) $batch = 5;
    ...
    $total_processed = (int)($_GET['tproc'] ?? 0);
    $total_generated = (int)($_GET['tgen'] ?? 0);
    ...
    foreach ($q->posts as $id) {
      $processed++;
      if (self::generate_poster((int)$id, $force)) {
        $generated++;
      }
    }
    ...
    $done = ($paged >= (int)$q->max_num_pages);
    $base = admin_url('options-general.php?page=rg-ffmpeg-video-poster');
    ...
    $url = add_query_arg([
      ...
      'batch'    => $batch,
      'force'    => $force ? 1 : 0,
      'tgen'     => $total_generated,
      'tproc'    => $total_processed,
    ], admin_url('admin-post.php?action=rg_ffmpeg_regen_all&_wpnonce=' . wp_create_nonce('rg_ffmpeg_regen_all')));
  ...
  /**
   * WP-CLI: wp rg-video-poster regen --force --batch=50
   */
  public static function cli_regen($args, $assoc_args): void {
    ...
    if (!self::exec_available()) {
      \WP_CLI::error('exec() ist nicht verfügbar. Poster-Generierung nicht möglich.');
      return;
    }
    ...
    do {
      ...
      \WP_CLI::log(sprintf('Batch %d/%d – processed: %d, generated: %d', $paged, (int)$q->max_num_pages, $total_processed, $total_generated));
      $paged++;
    } while ($paged <= (int)$q->max_num_pages);

    \WP_CLI::success(sprintf('Fertig. Verarbeitet: %d Videos, neu erzeugt: %d Poster.', $total_processed, $total_generated));
  }
  ...
  public static function render_settings_page(): void {
    ...
    // Progress / done notices
    $regen = sanitize_text_field($_GET['rg_regen'] ?? '');
    if ($regen === 'running') {
      echo '<div class="notice notice-info"><p><strong>Poster-Generierung läuft…</strong> Bitte Tab offen lassen. Es wird in Batches verarbeitet.</p></div>';
    } elseif ($regen === 'done') {
      $gen = (int)($_GET['gen'] ?? 0);
      $proc = (int)($_GET['proc'] ?? 0);
      echo '<div class="notice notice-success"><p><strong>Fertig ✅</strong> Verarbeitet: ' . esc_html((string)$proc) . ' Videos, neu erzeugt: ' . esc_html((string)$gen) . ' Poster.</p></div>';
    }

    echo '<h2>Bulk: Poster für bestehende Videos erzeugen</h2>';
    echo '<p>Damit kannst du alle bereits vorhandenen Video-Uploads (MP4/MOV) nachträglich durch <code>ffmpeg</code> laufen lassen und Poster erzeugen. Es wird in kleinen Paketen verarbeitet, damit es nicht zu Timeouts kommt.</p>';
    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
    echo '<input type="hidden" name="action" value="rg_ffmpeg_regen_all" />';
    wp_nonce_field('rg_ffmpeg_regen_all');
    echo '<label style="display:inline-block; margin-right:16px;"><input type="number" name="batch" min="5" max="100" value="20" style="width:90px;" /> <span>Batch-Größe</span></label>';
    echo '<label style="display:inline-block; margin-right:16px;"><input type="checkbox" name="force" value="1" /> <span>Auch neu erzeugen, wenn schon ein Poster existiert</span></label>';
    submit_button('Jetzt alle Videos verarbeiten', 'secondary', 'submit', false);
    echo '</form>';
