...
(function ($) {
  ...
  var BuddyBossActivitySharing = {
    ...
    submitSharedActivity: function submitSharedActivity(activityId, customMessage, privacy) {
      ...
      console.log('[BuddyBoss Share RL Debug] Topic selector elements found:', topicSelector.length);
      if (topicSelector.length) {
        hasTopicSelector = true;
        var selectedTopic = topicSelector.find('a.selected');
        console.log('[BuddyBoss Share RL Debug] Selected topic links:', selectedTopic.length);
        topicId = selectedTopic.data('topic-id') || 0;
        console.log('[BuddyBoss Share RL Debug] Final values - has_topic_selector:', hasTopicSelector, ', topic_id:', topicId);
      }
      $.ajax({
        url: buddybossSharingFrontend.ajaxUrl,
        type: 'POST',
        ...
        success: function success(response) {
          if (response.success) {
            ...
            if (response.data.activity && typeof bp !== 'undefined' && bp.Nouveau && bp.Nouveau.inject) {
