...
class MC4WP_Ecommerce_Command extends WP_CLI_Command
{
     ...
     */
    public function delete_order($args, $assoc_args = [])
    {
        $order_id = (int) $args[0];
     ...
     */
    public function add_orders($args, $assoc_args = [])
    {
     ...
     */
    public function add_products($args, $assoc_args = [])
    {
        $offset = empty($assoc_args['offset']) ? 0 : (int) $assoc_args['offset'];
        $limit = empty($assoc_args['limit']) ? 1000 : (int) $assoc_args['limit'];
        $delay = empty($assoc_args['delay']) ? 0 : (int) $assoc_args['delay'] * 1000;
     ...
     */
    public function add_product($args, $assoc_args = [])
    {
     ...
     */
    public function delete_product($args, $assoc_args = [])
    {
        $product_id = (int) $args[0];
    ...
    public function process_queue($args, $assoc_args = [])
    {
        ...
        while (($job = $queue->get())) {
            ...
            try {
                ...
                $success = call_user_func_array([ $worker, $job->data['method'] ], $job->data['args']);
            } catch (Error $e) {
